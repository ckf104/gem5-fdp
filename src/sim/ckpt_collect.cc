...
void addCkpt(uint64_t startnum, uint64_t length, uint64_t intregs[],
             uint64_t fpregs[], uint64_t pc, uint64_t npc,
             uint64_t instinfo[])
{
  uint64_t warmup = ckptsettings.ctrls[ckptidx-1].warmup;
  pendingCkpts.push_back(new CkptInfo(startnum, length, warmup, intregs,
      fpregs, pc, npc, ckptsettings.benchname, instinfo));
}
...
bool isCkptStart(uint64_t simNum, uint64_t &length)
{
  ...
  if (simNum == ckptsettings.ctrls[ckptidx].start) {
    length =
        ckptsettings.ctrls[ckptidx].end - ckptsettings.ctrls[ckptidx].start;
    ckptidx++;
    return true;
  }
  else{
...
uint64_t read_floads(FILE *p)
{
  ...
  if (compressTag-0x123456 == 1){
    ...
    printf(
      "the first load information is saved with data map and compressed by "
      "fastlz\n");
    while (1) {
      compsize = 0;
...
void ckpt_insert_syscall(uint32_t sysidx)
{
  ...
  NewSyscallInfo *infos = (NewSyscallInfo *)(
      infoaddr + sysidxs[sysidx] * sizeof(NewSyscallInfo));
