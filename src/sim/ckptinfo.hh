...
class CkptInfo
{
  public:
    ...
    CkptInfo(uint64_t startnum, uint64_t length, uint64_t warmup,
         uint64_t intregs[], uint64_t fpregs[], uint64_t pc,
         uint64_t npc, char filename[], uint64_t instinfo[]){
  ...
  public:
    ...
    void add_sysexe(uint64_t pc, uint64_t ret, uint64_t bufaddr,
    ...
    uint64_t getRange(map<uint64_t, uint8_t *> &addrs,
