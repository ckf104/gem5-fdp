...
void CkptInfo::add_sysenter(uint64_t pc, uint32_t sysnum,
...
void CkptInfo::add_sysexe(uint64_t pc, uint64_t ret, uint64_t bufaddr,
...
uint64_t CkptInfo::getRange(map<uint64_t, uint8_t *> &addrs,
                            vector<CodeRange> &ranges)
{
                          ...
                          uint64_t instinfo[])
{
  ...
  printf("preAccess size: %lu\n", preAccess.size());
  printf("textAccess size: %lu\n", textAccess.size());

  this->textsize = this->getRange(this->textAccess, this->textrange);
  this->memsize = this->getRange(this->preAccess, this->memrange);

  size_t len = textrange.size();
  uint64_t textend = textrange[len-1].addr + textrange[len-1].size;
...
void CkptInfo::saveDetailInfo()
{
  ...
  if (p == NULL){
    ...
    fprintf(p,
      "memory layout, brkpoint: 0x%lx, stack_base: 0x%lx, mapend: 0x%lx\n",
      ckptsettings.brk_point, ckptsettings.stack_base,
...
uint32_t dectect_Sysinfos(vector<SyscallInfo> &sysinfos, uint32_t idx,
                         set<uint64_t> &prepc, set<uint32_t> &prenum,
                         vector<uint32_t> &newinfos)
{
  uint32_t res=0;
  if (prepc.find(sysinfos[idx].pc) == prepc.end() ||
      prenum.find(sysinfos[idx].num) == prenum.end()) {
...
void CkptInfo::saveSysInfo(FILE *p)
{
  ...
  for (int i=0;i<sysinfos.size();i++) {
    if (sysinfos[i].data.size() != 0){
        sys_idxs.push_back(
          dectect_Sysinfos(sysinfos, i, prepc1, prenum1, newsysinfos1));
      num1 ++;
    }
    else{
        sys_idxs.push_back(
          dectect_Sysinfos(sysinfos, i, prepc, prenum, newsysinfos));
    }
  }
  ...
  for (int i=0; i<newsysinfos.size(); i++) {
    ...
    mem[4] = (sysinfos[idx].hasret) ?
      (invalid_addr<<8) + 1 : (invalid_addr<<8) + 0;
    fwrite(&mem[0], sizeof(uint64_t), 5, p);
  }
  ...
  for (int i=0; i<newsysinfos1.size(); i++) {
    ...
    fwrite(&mem[0], sizeof(uint64_t), 5, p);
    data_addr = data_addr + sysinfos[idx].data.size() + sizeof(uint64_t);
    //add the bufaddr information size
  }
